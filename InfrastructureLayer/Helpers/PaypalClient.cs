...
namespace InfrastructureLayer.Helpers
{
    public class PaypalClient
    {
        ...
        {
            var auth = Convert.ToBase64String(Encoding.UTF8.GetBytes($"{PayPalClientID}:{PayPalSecret}"));
            ...
            {
                RequestUri = new Uri($"{PayPalUrl}/v1/oauth2/token"),
        ...
        public async Task<CreateOrderResponse> CreateOrder(string value, string currency, string reference)
        {
            ...
            var httpResponse = await httpClient.PostAsJsonAsync($"{PayPalUrl}/v2/checkout/orders", request);
        ...
        public async Task<CaptureOrderResponse> CaptureOrder(string orderId)
        {
            ...
            var httpResponse = await httpClient.PostAsync($"{PayPalUrl}/v2/checkout/orders/{orderId}/capture", httpContent);
